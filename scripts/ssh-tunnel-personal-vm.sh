#!/usr/bin/env bash
# -----------------------------------------------------------------------------
# Зачем: открыть в браузере на ПК сервисы, которые слушают только на персональной ВМ / сервере БД.
# Когда: нужен доступ к приложению (лаб. 6/7) или pgAdmin (лаб. 7) без публикации портов в интернет.
# Где: запуск на своём компьютере; держите терминал открытым, Ctrl+C гасит туннели.
# Типовой сценарий: ./scripts/ssh-tunnel-personal-vm.sh → в браузере http://127.0.0.1:8080 (приложение).
# -----------------------------------------------------------------------------
...
echo ""
echo "Подсказка: этот терминал после подключения — сессия на ВМ (команды выполняются там)."
echo "  Локальные команды — во втором окне терминала. Или только туннель без shell: добавьте флаг -N"
echo "  (например: $0 -N). Тогда приглашения не будет, пока не нажмёте Ctrl+C — это нормально."
echo ""
