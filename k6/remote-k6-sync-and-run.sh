#!/usr/bin/env bash
# Синхронизация сценариев k6 на общую машину и запуск lab6 sweep по SSH.
...
echo "==> scp k6-сценариев -> ${REMOTE}:${K6_REMOTE_DIR}/k6/"
scp -P "${K6_SSH_PORT}" \
  "${ROOT}/k6/cinema-lab6-constant.js" \
  "${ROOT}/k6/run-lab6-ratio-sweep.sh" \
  "${REMOTE}:${K6_REMOTE_DIR}/k6/"

echo "==> chmod +x sweep на удалённой стороне"
"${SSH[@]}" "mkdir -p ${K6_REMOTE_DIR}/k6 && chmod +x ${K6_REMOTE_DIR}/k6/run-lab6-ratio-sweep.sh"
