# Лабораторная работа №6: «Кинотеатр» (Cinema)
...
**Фокус README и сдачи — лаб. 6** (текст **`ТЗ_6лаба.txt`**, шпаргалка **`docs/lab6-one-pager.md`**, отчёт по графикам **`docs/lab6-report-png_k6.md`**): развёртывание на **учебной ВМ**, образ в **Docker Hub** (**`docker compose push` / `pull`**), **лимиты CPU/RAM** сервиса `app`, **переменные окружения** для БД и Tomcat, отключение **`spring.jpa.show-sql`**, доступ с ПК через **SSH-туннель** (**`scripts/ssh-tunnel-personal-vm.sh`**), нагрузка **k6** с **постоянными VU** и смесьями **5/95, 50/50, 95/5**, графики «время отклика vs лимит CPU». Раздел **«Лабораторная работа №6»** ниже — основной по объёму.

**Тот же репозиторий** содержит материалы предыдущих работ: **лаб. 5** — сидирование (**`tools/seed_rest_data.py`**, **`DELETE /api/admin/clear/*`**); **лаб. 4** — k6 с ростом VU (**`k6/run-sweep.sh`**); **лаб. 3** — контейнеризация и Flyway. Эти части остаются в документе как справка и для подготовки данных перед прогонами лаб. 6.

**Навигация:** **эндпоинты**, **требования**, **быстрый старт** (Docker), обновление образа, **профили**, **миграции**, **структура**, **скрипты Bash** — общая база проекта; затем **лаб. 6** (ТЗ, запуск «с нуля», забыли детали, сдача преподавателю, порты, env, SSH, k6); **лаб. 5** (сидирование); **лаб. 4** (k6); описание проекта, **SQL**, **порты**.
