...
// Замер времени обработки каждого запроса под /api/**: старт до контроллера, стоп после.
public class ObservabilityHttpInterceptor implements HandlerInterceptor {

    // В request кладём результат start(), в afterCompletion тем же ключом читаем.
    ...
    public void afterCompletion(
        ...
        // Исключение из цепочки или HTTP 5xx — для метрик считаем ошибкой; 4xx без исключения — успех.
    ...
    // Имя для метрик: controller.<КлассКонтроллера>.<javaМетод>.
